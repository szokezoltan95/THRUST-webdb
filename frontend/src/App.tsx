...
function GimbalPreview({ configuration, selectedColor, onSelect }: { configuration: ScopeConfiguration; selectedColor: string; onSelect: (key: string) => void }) {
  ...
  const gimbal = (x: number, prefix: string) => {
    ...
    return <g key={prefix} onClick={() => onSelect("gimbal_background")} className="preview-clickable">
      <rect x={x} y="52" width="220" height="220" rx="3" fill={color("gimbal_background")} />
      <g fill="none" stroke={color("grid_color")} strokeLinecap="square" strokeLinejoin="miter" onClick={(event) => { event.stopPropagation(); onSelect("grid_color"); }} className="preview-clickable">
      ...
      <circle cx={x + 110} cy="162" r={zoneRadius} fill={color("zone_idle_fill")} stroke={color("zone_idle_outline")} strokeWidth={8 * scale} onClick={(event) => { event.stopPropagation(); onSelect("zone_idle_fill"); }} className="preview-clickable" />
      <circle cx={x + 110} cy="162" r={stickRadius} fill={color("stick_fill")} stroke={color("stick_outline")} strokeWidth={6 * scale} onClick={(event) => { event.stopPropagation(); onSelect("stick_fill"); }} className="preview-clickable" />
    </g>;
  };
...
function TestEditor({ test, onClose, onSaved }: { test: TestDefinition; onClose: () => void; onSaved: (test: TestDefinition) => void }) {
  ...
  return <div className="editor-backdrop"><section className="test-editor-window"><header className="editor-header"><div><div className="eyebrow">EDITOR TESTU · ROZPRACOVANÁ VERZIA</div><h2>{test.name}</h2><p className="muted">{test.test_code} · v{test.version}</p></div><button type="button" className="quiet compact" onClick={onClose}>Zavrieť</button></header><div className="editor-layout"><div className="editor-controls">
